...
var warn = AFRAME.utils.debug('components:raycaster2:warn');
...
const elId = i => i.object3D.uuid.substr(0,4);
 ...
 * @member {object} raycaster2 - three.js Raycaster.
 */
AFRAME.registerComponent('raycaster2', {
  schema: {
    ...
    maxObjects: {default: -1},
    uniqueElements: {default: false}
  },
  ...
  /**
   * Create or update raycaster2 object.
   */
  update: function (oldData) {
    var data = this.data;
    var el = this.el;
    var raycaster2 = this.raycaster2;

    // Set raycaster2 properties.
    raycaster2.far = data.far;
    raycaster2.near = data.near;
    ...
    if (data.objects !== oldData.objects && !OBSERVER_SELECTOR_RE.test(data.objects)) {
      warn('[raycaster2] Selector "' + data.objects +
           '" may not update automatically with DOM changes.');
    }

    if (!data.objects) {
      warn('[raycaster2] For performance, please define raycaster2.objects when using ' +
           'raycaster2 or cursor components to whitelist which entities to intersect with. ' +
           'e.g., raycaster2="objects: [data-raycastable]".');
    }
  ...
  checkIntersections: function () {
    ...
    this.raycaster2.intersectObjects(this.objects, true, rawIntersections);

    // Only keep intersections against objects that have a reference to an entity.
    intersections.length = 0;
    intersectedEls.length = 0;
    const uniqueEls = new WeakSet();
    for (i = 0; i < rawIntersections.length; i++) {
      ...
      if (intersection.object.el) {
        if(!data.uniqueElements || (data.uniqueElements && !uniqueEls.has(intersection.object.el))) {
          intersections.push(intersection);
          intersectedEls.push(intersection.object.el);
          uniqueEls.add(intersection.object.el);
        }
      }
    }

    if(data.maxObjects >= 0) {
      intersections.length = Math.min(intersections.length, data.maxObjects);
      intersectedEls.length = Math.min(intersectedEls.length, data.maxObjects);
    }
  ...
  /**
   * Update origin and direction of raycaster2 using entity transforms and supplied origin or
   * direction offsets.
   */
  updateOriginDirection: (function () {
    ...
    return function updateOriginDirection () {
      ...
      if (data.useWorldCoordinates) {
        this.raycaster2.set(data.origin, data.direction);
        return;
      }
      ...
      // three.js raycaster2 direction is relative to 0, 0, 0 NOT the origin / offset we
      ...
      this.raycaster2.set(originVec3, direction);
    };
  })(),

  /**
   * Create or update line to give raycaster2 visual representation.
   * Customize the line through through line component.
   * We draw the line in the raycaster2 component to customize the line to the
   * raycaster2's origin, direction, and far.
   *
   * Unlike the raycaster2, we create the line as a child of the object. The line will
   * be affected by the transforms of the objects, so we don't have to calculate transforms
   * like we do with the raycaster2.
   *
   * @param {number} length - Length of line. Pass in to shorten the line to the intersection
   *   point. If not provided, length will default to the max length, `raycaster2.far`.
   */
